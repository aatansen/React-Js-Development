...
const moviesArr = [
  ...
  },
  {
    ...
    ticketPrice: 380,
    posterURL:
      "https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcSXHzsHwErYlOX4zA0IqmHWyfvTIzut-r8OvzNEFfRXyFPP12BI",
  },
  {
    title: "Attorney Woo",
    year: 2022,
  ...
  for (let i = 0; i < moviesArr.length; i++) {
    ...
    movieElement.innerHTML = `
          ...
          <div class="card-body">
            ...
            <h5 class="text-danger fw-bold">Price : ${moviesArr[i].ticketPrice} ৳.</h5>
  ...
  for (let i = 0; i < bookTickets.length; i++) {
    bookTickets[i].addEventListener("click", function () {
      ...
      for (let j = 0; j < moviesArr.length; j++) {
        const element = moviesArr[j];
        if (i === j) {
          // console.log(moviesArr[j]);
          const title = moviesArr[i].title;
          const year = moviesArr[i].year;
          const price = moviesArr[i].ticketPrice;
